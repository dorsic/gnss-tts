...
void configure_clocks() {
    ...
    #endif    
    if (pdiv_ext_ref) {
        uint8_t source_gpio = EI_CLK_GPIO;
...
void scpi_errorhandler(SCPI_C commands, SCPI_P parameters, SCPI_I interface) {
  switch(cm_instrument.last_error){
    ...
    case cm_instrument.ErrorCode::NoError:
      ...
      break;
    case cm_instrument.ErrorCode::Unsupported:
      interface.putchars("Unsupported by device\n");
  }
  cm_instrument.last_error = cm_instrument.ErrorCode::NoError;    
}

void identify(SCPI_C commands, SCPI_P parameters, SCPI_I interface) {
    interface.putchars("PicoDIV in Counter Module v1.0, v1.1\n");
}
...
void unsupported(SCPI_C commands, SCPI_P parameters, SCPI_I interface) {
    cm_instrument.last_error = cm_instrument.ErrorCode::Unsupported;
    scpi_errorhandler(commands, parameters, interface);
}
/*
...
void initialize() {
    ...
    cm_instrument.SetCommandTreeBase(":DIVider:TRIGger");        
        cm_instrument.RegisterCommand(":ENABle", &unsupported);             // param <ON | 1> | <OFF | 0>
        cm_instrument.RegisterCommand(":DELay", &unsupported);               // param delay in clock cycles
        cm_instrument.RegisterCommand(":FREQuency", &unsupported);        // param freq in Hz
        cm_instrument.RegisterCommand(":PULSelength", &unsupported);   // param <high portion in ns> | <p%>
        cm_instrument.RegisterCommand(":SYNChronization", &unsupported);        // param <ON | 1> | <OFF | 0>
    cm_instrument.SetCommandTreeBase(":TIC");
        cm_instrument.RegisterCommand(":STARt", &unsupported);        // param <CH1 | CH2 | TRIG | REF>
        cm_instrument.RegisterCommand(":STARt?", &unsupported);
        cm_instrument.RegisterCommand(":STOP", &unsupported);          // param <CH1 | CH2 | TRIG | REF>
        cm_instrument.RegisterCommand(":STOP?", &unsupported);
        cm_instrument.RegisterCommand(":IMPedance", &unsupported);      // param <50 | LOW> | <1M | HIGH>
