...
//Do nothing function
void DefaultErrorHandler(SCPI_C c, SCPI_P p, SCPI_Interface i) { }
...
/*!
 ...
 @see GetMessage
*/
void SCPI_Parser::Execute(SCPI_Interface interface, char* message) {
  while (message != NULL) {
 ...
 //       printf("Executing message (%d)\n", code);
    ...
    if (code==255) {
      last_error = ErrorCode::CommandOverflow;
      (*callers_[SCPI_MAX_COMMANDS])(commands, parameters, interface);
    }
...
/*!
 ...
 @see Execute
*/
void SCPI_Parser::ProcessInput(SCPI_Interface interface, const char* term_chars) {
  char* message = this->GetMessage(interface, term_chars);
  if (message != NULL) {
//    printf("Executing command %s\n", message);
    this->Execute(interface, message);
  }
}

void SCPI_Parser::Initialize() {
    #ifdef COM_UART
    if (tx_offset == -1)
    ...
    gpio_set_function(SPIRX_GPIO, GPIO_FUNC_SPI);
    gpio_set_function(SPITX_GPIO, GPIO_FUNC_SPI);
    gpio_set_function(SPICLK_GPIO, GPIO_FUNC_SPI);
    // Configure Chip Select
    gpio_init(SPICS_GPIO); // Initialise CS Pin
    gpio_set_dir(SPICS_GPIO, GPIO_IN); // Set CS as output
    #endif
}
...
*/
char* SCPI_Parser::GetMessage(SCPI_Interface interface, const char* term_chars) {
  char ch = interface.getchar_non_blocking();
  while (ch != '\0') {
    //Read the new char
    msg_buffer_[message_length_] = ch;
    ++message_length_;
    time_checker_ = time_us_32();

    if (message_length_ >= SCPI_BUFFER_LENGTH){
      //Call ErrorHandler due BufferOverflow
      last_error = ErrorCode::BufferOverflow;
      (*callers_[SCPI_MAX_COMMANDS])(SCPI_C(), SCPI_P(), SCPI_I());
      message_length_ = 0;
      return NULL;
    }
    ...
    ch = interface.getchar_non_blocking();
  }
  ...
  if ((time_us_32() - time_checker_) > SCPI_TIMEOUT) {
      //Call ErrorHandler due Timeout
      printf("timediff: %llu, message_length_ %d, (%s)\n", time_us_32() - time_checker_, message_length_, msg_buffer_);
      last_error = ErrorCode::Timeout;
      (*callers_[SCPI_MAX_COMMANDS])(SCPI_C(), SCPI_P(), SCPI_Interface());
      message_length_ = 0;
      return NULL;
  }
...
void SCPI_Parser::PrintDebugInfo() {
  printf("*** DEBUG INFO ***\n\n");
  printf("TOKENS (%i): ", tokens_size_);
  for (uint8_t i = 0; i < tokens_size_; i++) {
    printf(" %s", tokens_[i]);
  }
  printf("\nVALID CODES:");
  for (uint8_t i = 0; i < codes_size_; i++) {
    printf(" %i", valid_codes_[i]);
  }
  printf("\n*******************\n");
}

SCPI_Interface::SCPI_Interface() {}

char SCPI_Interface :: getchar_non_blocking() {
    #ifdef COM_UART
//    if (gpio_get(UART_CS)) {
//        ch = uart_rx_program_getc(UART_PIO, UART_RX_SM);
//    }
  
    int ch = getchar_timeout_us(0);
    char c = (ch != PICO_ERROR_TIMEOUT)? (char)ch: '\0';
    if (c == '\0') {
      uint32_t st = time_us_32();
      c = uart_rx_program_getc(UART_RX_PIO, UART_RX_SM);
      while ((c == '\0') && ((time_us_32()-st) < UART_TIMEOUT_US)) {
        c = uart_rx_program_getc(UART_RX_PIO, UART_RX_SM);
      }
      pio_sm_clear_fifos(UART_RX_PIO, UART_RX_SM);
      return c;
    }
    return c;
    #endif
    #ifdef COM_SPI
    uint8_t b[1];
//    if (spi_is_readable(COMSPI)) {
        spi_read_blocking(COMSPI, 0, b, 1);
        printf("%s", (char)b[0]);
        return (char)b[0];
//    }
//    printf(".");
...
//    if (gpio_get(UART_CS)) {
        ...
        pio_sm_restart(UART_TX_PIO, UART_TX_SM);
        pio_sm_set_enabled(UART_TX_PIO, UART_TX_SM, true);  
        uart_tx_program_puts(UART_TX_PIO, UART_TX_SM, message);
...
//    }
    ...
    for (uint8_t i = 0; i < strlen(message); i++) {
      b[i] = (uint8_t)message[i];
    }
    if (spi_is_writable(COMSPI))
        spi_write_blocking(COMSPI, b, strlen(message));
    #endif
    printf(message);
}
